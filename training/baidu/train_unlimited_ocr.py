"""LoRA fine-tuning for Baidu Unlimited-OCR on the handwritten-math -> LaTeX
dataset (the same 9_split used for GLM-OCR). Runs in the BaiduOCR venv.

The model's forward() computes CrossEntropyLoss from `labels`, and the inner
forward splices vision features into the embeddings via images_seq_mask. We build
each training example exactly the way infer() builds its inputs (reusing the
model module's own preprocessing helpers), then append the target LaTeX tokens
and mask the prompt/image positions with -100 so loss is only on the target.

Memory: 3B model on 12GB -> LoRA on the DeepSeek-V2 decoder + bf16 + gradient
checkpointing + batch 1 (use --grad-accum for a larger effective batch), or
--qlora for 4-bit base weights if it OOMs. Vision features are computed under
no_grad inside the model, so only the decoder LoRA trains.

Writes training_metrics.json for the dashboard's "Fine-tune Baidu" tab.
"""
...
def main():
    ...
    ap.add_argument("--output-dir", default=r"D:\Claude Code\BaiduOCR\finetune\baidu-ocr-math-v1")
    ...
    ap.add_argument("--max-target-len", type=int, default=1024)
    ...
    ap.add_argument("--early-stopping-patience", type=int, default=0, help="stop if val loss doesn't improve for N evals (0=off)")
